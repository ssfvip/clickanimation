...
public class MyImageView extends ImageView {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                float X = event.getX();
                float Y = event.getY();
                mScaleHansler.sendEmptyMessage(SCALE_REDUCE_INIT);
                break;
            case MotionEvent.ACTION_UP:
                mScaleHansler.sendEmptyMessage(SCALE_ADD_INIT);
                break;
        }
        return true;
    }
    ...
    // 控制缩放的handler
    private Handler mScaleHansler = new Handler(){
            ...
            switch (msg.what){
                case SCALE_REDUCE_INIT:
                    if (!isFinish){
                        ...
                        beginScale(matrix, s);
                        mScaleHansler.sendEmptyMessage(SCALING);
                    }
                    break;
                case SCALING:
                    beginScale(matrix, s);
                    if (count < 4){
                        ...
                        isFinish = true;
                        if (MyImageView.this.mOnViewClickListener != null && !isClick){
                            isClick = true;
